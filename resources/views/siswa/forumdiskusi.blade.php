    ...
    <div class="row justify-content-center">
        <div class="col-md-8 mb-4">
            <div class="card shadow-sm border-0">
                <div class="card-body">
                    <form action="{{ route('siswa.post.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center">
                    ...
                    @if($post->user_id == Auth::id())
                        <form action="{{ route('siswa.post.destroy', $post->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                ...
                <div class="card-footer bg-white border-0">
                    @foreach($post->comments as $comment)
                        <div class="comment-box">
                            ...
                            @if($comment->user_id == Auth::id())
                                <form action="{{ route('siswa.comment.destroy', $comment->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <div class="reply-form mt-2" style="display: none;">
                                <form action="{{ route('siswa.comment.reply', [$post->id, $comment->id]) }}" method="POST">
                                    @csrf
                    ...
                    <form action="{{ route('siswa.comment.store', $post->id) }}" method="POST" class="d-flex align-items-center mt-3">
                        @csrf
